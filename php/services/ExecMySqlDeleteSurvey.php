...
//
	...
	$id_to_delete = $_GET['survey_id'];
	$conn->begin_transaction();
	try {
		$sql_query1 = "DELETE FROM usercompletedsurveys where userId= ". $id_to_delete;
		$statement = $conn->prepare($sql_query1);
		$statement->execute();
		$statement->close();

		$sql_query2 = "DELETE FROM usercompletedpolls where userId= ". $id_to_delete;
		$statement = $conn->prepare($sql_query2);
		$statement->execute();
		$statement->close();

		$sql_query3 = "DELETE FROM users where id= ". $id_to_delete;
